...
import type { CategoriaRow, CuentaRow } from "@/lib/local-db";
import { getCatalog, saveCatalog } from "@/lib/local-db";
import { formatInputNumber, parseMoney } from "@/lib/money";
...
export function CatalogManager() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        const cats: UiCategoria[] = c.categorias.map((c) => ({
          tipo: c.tipo || "Gasto",
          categoria: c.categoria,
          subcategoria: c.subcategoria,
          presupuesto: c.presupuesto != null && c.presupuesto !== 0 ? formatInputNumber(c.presupuesto) : "",
        }));
        const accs: UiCuenta[] = c.cuentas.map((c) => ({
          nombre: c.nombre,
          tipo: c.tipo ?? "",
          saldo: c.saldo != null ? formatInputNumber(c.saldo) : "",
        }));
        setCategorias(cats);
        setCuentas(accs);
        setError(null);
        ...
        <>
          <section className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            <div className="mb-3">
              ...
              <p className="mt-0.5 text-xs text-zinc-400">
                Se guardan en tu archivo .cvw y se usan al registrar movimientos.
              </p>
            </div>
            ...
            <button
              onClick={() => setCategorias([...categorias, { tipo: "Gasto", categoria: "", presupuesto: "" }])}
          ...
          <div className="flex items-center justify-between gap-3 rounded-2xl border border-zinc-200 bg-white px-4 py-3 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            <p className="text-xs text-zinc-400 dark:text-zinc-500">
              Se escribe directo en tu archivo .cvw.
            </p>
            <button
              onClick={save}
  ...
  async function save() {
    ...
    setError(null);
    try {
      const cats: CategoriaRow[] = categorias.map((c) => ({
        tipo: c.tipo === "Ingreso" ? "Ingreso" : "Gasto",
        categoria: c.categoria,
        subcategoria: c.subcategoria,
        presupuesto: c.presupuesto.trim() !== "" ? parseMoney(c.presupuesto) : undefined,
      }));
      const accs: CuentaRow[] = cuentas.map((c) => ({
        nombre: c.nombre,
        tipo: c.tipo,
        saldo: c.saldo.trim() !== "" ? parseMoney(c.saldo) : undefined,
      }));
      await saveCatalog(cats, accs);
